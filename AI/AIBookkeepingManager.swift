...
final class AIBookkeepingManager: ObservableObject {
    ...
    var apiKey: String { KeychainService.read("aiAPIKey") ?? "" }
    ...
    使用完整复式记账规则：支出借记 Expenses:分类、贷记资产或负债账户；收入借记资产或负债账户、贷记 Income:分类；转账只在两个资产/负债账户之间移动。信用卡和花呗是负债，消费时贷记负债，偿还时借记负债、贷记付款账户。
    ...
    func parse(text: String) async throws -> TransactionProposal {
        ...
        let proposal = endpoint != nil ? try await request(input: ["文本": input, "系统提示": Self.systemPrompt]) : try localParse(input)
        guard proposal.postings.allSatisfy({ $0.amount == $0.amount.roundedToCents }), proposal.postings.reduce(Decimal.zero, { $0 + $1.amount }).roundedToCents == 0 else { throw AIBookkeepingError.unbalancedProposal }
        lastReply = "绯儿已经整理好了，请确认这笔分录。"
        return proposal
    }
    ...
    private func localParse(_ text: String) throws -> TransactionProposal {
        ...
        guard let match = text.range(of: pattern, options: .regularExpression), let amount = Decimal(string: String(text[match]))?.roundedToCents else { throw AIBookkeepingError.invalidResponse }
    ...
    private func request(input: [String: String]) async throws -> TransactionProposal {
        ...
        guard proposal.postings.reduce(Decimal.zero, { $0 + $1.amount }).roundedToCents == 0 else { throw AIBookkeepingError.unbalancedProposal }
        return proposal
    }
}

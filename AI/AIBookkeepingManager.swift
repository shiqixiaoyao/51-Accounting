...
enum AIBookkeepingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidResponse:
            return "绯儿没有返回有效的记账结果，请检查接口设置后重试。"
        case .unbalancedProposal:
            return "绯儿生成的分录不平衡，未创建交易。"
        }
    }
}
...
final class AIBookkeepingManager: ObservableObject {
    ...
    var provider: AIProvider {
        get {
            AIProvider(rawValue: UserDefaults.standard.string(forKey: "aiProvider") ?? "") ?? .poke
        }
        set {
    ...
    var endpoint: URL? {
        URL(string: (UserDefaults.standard.string(forKey: "aiEndpoint") ?? "").trimmingCharacters(in: .whitespacesAndNewlines))
    }
    ...
    func parse(text: String) async throws -> TransactionProposal {
        ...
        let proposal = try await request(input: ["文本": input])
        guard proposal.postings.count >= 2,
              proposal.postings.allSatisfy({ $0.amount == $0.amount.roundedToCents }),
    ...
    private func request(input: [String: String]) async throws -> TransactionProposal {
        ...
        request.httpBody = try JSONSerialization.data(withJSONObject: ["provider": provider.rawValue, "input": input])
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            throw AIBookkeepingError.invalidResponse
        }
        return try JSONDecoder().decode(TransactionProposal.self, from: data)
    }
}

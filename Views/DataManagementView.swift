...
struct DataManagementView: View {
    ...
    var body: some View {
        Form {
            Section("导出") {
                ...
                Button("导出 JSON 完整备份") {
                    do { prepare(try DataTransferService.backup(accounts: accounts, categories: categories, transactions: transactions), name: "51-backup.json", share: false) }
                    catch { errorMessage = error.localizedDescription }
                }
                Button("复制 Beancount 到剪贴板") {
                    UIPasteboard.general.string = String(data: DataTransferService.beancount(transactions), encoding: .utf8)
                    message = "已复制到剪贴板"
                }
            }
            Section("导入与恢复") {
                Picker("冲突处理", selection: $conflict) {
                    ForEach(ImportConflict.allCases) { option in Text(option.rawValue).tag(option) }
                }
    ...
    private func prepare(_ data: Data, name: String, share: Bool) {
        exportDocument = AccountingFileDocument(data: data)
        exportName = name
        showingExporter = true
        if share { shareItems = [data] }
    }
    ...
    private func importFile(_ result: Result<[URL], Error>) {
        do {
            ...
            if url.pathExtension.lowercased() == "json" {
                try DataTransferService.restore(DataTransferService.readBackup(data), in: modelContext, conflict: conflict)
                message = "JSON 备份已恢复"
            } else {
    ...
    private func importBean(_ data: Data) throws -> Int {
        ...
        for block in text.components(separatedBy: "\n\n") {
            ...
            guard let header = lines.first, header.count >= 10,
                  let date = ISO8601DateFormatter().date(from: String(header.prefix(10)) + "T00:00:00Z") else { continue }
            ...
            if !postings.isEmpty && postings.reduce(Decimal.zero, { $0 + $1.amount }) == 0 {
                modelContext.insert(BookkeepingTransaction(date: date, payee: payee, postings: postings))
                count += 1
            }
        }

...
struct DataManagementView: View {
 ...
 var body: some View { Form { Section("导出") { Button("导出 Beancount (.bean)") { prepare(DataTransferService.beancount(transactions), name: "51-accounting.bean", share: true) }; Button("导出交易 CSV") { prepare(DataTransferService.csv(transactions), name: "51-transactions.csv", share: false) }; Button("导出 JSON 完整备份") { do { prepare(try DataTransferService.backup(accounts: accounts, categories: categories, transactions: transactions), name: "51-backup.json", share: false) } catch { errorMessage = error.localizedDescription } }; Button("复制 Beancount 到剪贴板") { UIPasteboard.general.string = String(data: DataTransferService.beancount(transactions), encoding: .utf8); message = "已复制到剪贴板" } }; Section("导入与恢复") { Picker("冲突处理", selection: $conflict) { ForEach(ImportConflict.allCases) { option in Text(option.rawValue).tag(option) } }; Button("从文件导入 JSON / Beancount") { showingImporter = true }; Text("JSON 可恢复账户、分类、交易和分录；Beancount 文件将按交易段落导入。导入前会校验借贷平衡。").font(.caption).foregroundStyle(.secondary) }; if !message.isEmpty { Text(message).foregroundStyle(.green) }; if !errorMessage.isEmpty { Text(errorMessage).foregroundStyle(.red) } }.navigationTitle("数据管理").fileExporter(isPresented: $showingExporter, document: exportDocument, contentType: .data, defaultFilename: exportName) { result in if case .failure(let e) = result { errorMessage = e.localizedDescription } }.fileImporter(isPresented: $showingImporter, allowedContentTypes: [.json, .plainText, .commaSeparatedText], allowsMultipleSelection: false) { result in importFile(result) }.sheet(isPresented: Binding(get: { !shareItems.isEmpty }, set: { if !$0 { shareItems = [] } })) { ActivityView(activityItems: shareItems) } }

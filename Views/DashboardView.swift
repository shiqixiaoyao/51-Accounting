...
struct DashboardView: View {
    ...
    private var summaryCard: some View { GlassCard(tint: .cyan) { VStack(alignment: .leading, spacing: 18) { Label("资产总览", systemImage: "chart.line.uptrend.xyaxis").foregroundStyle(.secondary); Text("¥\(NSDecimalNumber(decimal: balance).stringValue)").font(.system(size: 34, weight: .bold, design: .rounded).monospacedDigit()); Label("保持稳定的记账节奏", systemImage: "arrow.up.right").font(.caption).foregroundStyle(.green) } } }
    private var accountsCard: some View { GlassCard(tint: .purple) { VStack(alignment: .leading, spacing: 14) { Label("账户余额", systemImage: "wallet.pass.fill").font(.headline).foregroundStyle(.purple); if accounts.isEmpty { Text("添加账户后，即可在这里查看余额").foregroundStyle(.secondary) } else { ForEach(accounts.prefix(3)) { account in HStack { GradientIcon(systemName: account.isLiability ? "creditcard.fill" : "building.columns.fill", colors: [.purple, .cyan]); Text(account.name); Spacer(); Text(NSDecimalNumber(decimal: account.openingBalance).stringValue).monospacedDigit() } } } } } }

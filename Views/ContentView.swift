...
struct ContentView: View {
    ...
    private func applyPendingShortcutRoute() {
        ...
        switch route {
        case .addTransaction: showingAdd = true
        case .aiAccounting: selectedTab = 1
        }
    }
}
...
struct AppBackdrop: View {
    ...
    var body: some View {
        ZStack {
            Color.black
            LinearGradient(
                colors: [accent.opacity(0.17), .indigo.opacity(0.08), .black],
        ...
        content
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 24, style: .continuous)
                    .stroke(
                        LinearGradient(
                            colors: [tint.opacity(0.34), .white.opacity(0.06)],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        ),
                        lineWidth: 1
...
struct GradientIcon: View {
    ...
    var body: some View {
        Image(systemName: systemName)
            ...
            .background(
                LinearGradient(colors: colors.map { $0.opacity(0.26) }, startPoint: .topLeading, endPoint: .bottomTrailing),

...
struct AIServiceSettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("配置标准 LLM 服务后，AI 只生成待确认分录。").font(.caption).foregroundStyle(.secondary)
                    }
                }.padding(.vertical, 5)
            ...
            Section("服务配置") {
                ...
                TextField("HTTPS Endpoint", text: $endpoint).textInputAutocapitalization(.never).keyboardType(.URL)
                TextField("Model 名称", text: $model).textInputAutocapitalization(.never)
                SecureField("API Key（仅保存至 Keychain）", text: $apiKey)
            }
            Section("连接与安全") {
                Label("每个预设单独保存 Endpoint、Model 与 API Key。密钥仅保存在系统 Keychain。", systemImage: "lock.shield.fill").font(.footnote).foregroundStyle(.secondary)
                Button(action: save) { Label("保存当前服务配置", systemImage: "checkmark.circle") }
                Button { Task { await testConnection() } } label: {
                    if isTesting { Label("正在测试连接", systemImage: "arrow.triangle.2.circlepath") } else { Label("测试 \(provider.rawValue) 连接", systemImage: "checkmark.icloud") }
                }.disabled(isTesting)
            }
        ...
        .onAppear { load(provider: AIConfigurationStore.selectedProvider()) }
    }
    ...
    private func save() {
        do { try AIConfigurationStore.save(provider: provider, endpointText: endpoint, model: model, apiKey: apiKey); errorMessage = nil; message = "\(provider.rawValue) 配置已安全保存。" }
        catch { message = nil; errorMessage = error.localizedDescription }
    }
